...
const SendChallenge: React.FC<SendChallengeProps> = ({ onCancel }) => {
  ...
  const onFinish = ({ challengee, workout, message }: onFinishProps): void => {
  ...
  useEffect(() => {
    if (user) {
      getUserFriends(user.uid).then((res) => {
        getFriendsProfilesByIds(res).then((res) => {
          if (res) {
            setfriendsList((prev) => [...prev, ...res]);
          }
        });
      });
      getUserWorkouts(user.uid).then((response) => {
        if (response) {
      ...
      <Form onFinish={onFinish} form={form}>
        <div className="sendChallenge_inputs">
          ...
          <Form.Item label="Want send a short message with your challenge?" name="message">
            <Input placeholder="message..." maxLength={50}></Input>
          </Form.Item>
        </div>
